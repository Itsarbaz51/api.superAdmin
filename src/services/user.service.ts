...
class UserServices {
  ...
  static async getAllUsersByParentId(parentId: string): Promise<User[]> {
    if (!parentId) {
    ...
    // Check cache first
    const cacheKey = `users:parent:${parentId}`;
    const cachedUsers = await getCache<User[]>(cacheKey);

    if (cachedUsers) {
      logger.debug("Users by parent fetched from cache", {
        parentId,
        count: cachedUsers.length,
      });
      return cachedUsers;
    ...
    logger.debug("Users by parent fetched from database", {
      parentId,
      count: safeUsers.length,
    });
